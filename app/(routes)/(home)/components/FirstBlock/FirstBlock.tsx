import Reveal from "@/components/Shared/Reveal/Reveal";
import Image from "next/image";

export default function FirstBlock() {
  return (
    <div className="grid lg:grid-cols-2 lg:px-0 lg:py-14 xl:py-10 items-center">
      <Reveal className="p-6 lg:pl-40" position="bottom">
        <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold">
          Premium
          <span className="block">Rental Cars</span>
          en Chile
        </h1>
        <p className="text-lg mt-2 lg:mt-5 lg:text-xl max-w-sm">
          No te niegues el placer de conducir los mejores autos por todo el
          mundo, aquí y ahora.
        </p>
      </Reveal>
      <Reveal className="flex justify-end" position="right">
        <Image
          src="/images/LandingAuto1.jpg"
          alt="Rent Cars"
          width={800}
          height={800}
          priority
        />
      </Reveal>
    </div>
  );
}
